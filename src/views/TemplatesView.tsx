...
export function TemplatesView({ loading = false }: TemplatesViewProps) {
  ...
  useEffect(() => {
    if (templateListRef.current) autoAnimate(templateListRef.current, { duration: 200 });
  }, []);

  /* ── Store ── */
  const selectedRecords: EssaRecord[] = useMemo(() => {
    if (!records || records.length === 0 || selectedRows.size === 0) return [];
    return (records as EssaRecord[]).filter((r) =>
      selectedRows.has((r as unknown as { rowId: string }).rowId)
    );
  ...
  const previewRecord: EssaRecord | null = useMemo(() => {
    ...
    if (lastCheckedRowIdRef.current) {
      const found = selectedRecords.find(
        (r) => (r as unknown as { rowId: string }).rowId === lastCheckedRowIdRef.current
      );
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (recordSearch.trim()) {
      ...
      out = out.filter((r) => {
        ...
        return (
          String(rr.nombreSolicitante ?? '')
            .toLowerCase()
            .includes(q) ||
          String(rr.numeroCuenta ?? rr.cuenta ?? '')
            .toLowerCase()
            .includes(q) ||
          String(rr.radicadoEntrada ?? '')
            .toLowerCase()
            .includes(q)
        );
      });
    }
    if (recordStatusFilter === 'asignado')
      out = out.filter((r) => !!templateAssignments[(r as unknown as { rowId: string }).rowId]);
    else if (recordStatusFilter === 'sin_asignar')
      out = out.filter((r) => !templateAssignments[(r as unknown as { rowId: string }).rowId]);
    return out;
  ...
  const assignedCount = useMemo(
    () =>
      selectedRecords.filter(
        (r) => !!templateAssignments[(r as unknown as { rowId: string }).rowId]
      ).length,
    [selectedRecords, templateAssignments]
  );
  ...
  const handleAssignTemplate = () => {
    ...
    const conflicting = idsToAssign.filter(
      (id) => templateAssignments[id] && templateAssignments[id] !== selectedTemplate.id
    );
    if (conflicting.length > 0) {
      setConfirmOverwrite({ open: true, rowIds: idsToAssign, templateId: selectedTemplate.id });
      return;
    }
  ...
  if (!templates || templates.length === 0)
    return (
      ...
      >
        ...
        >
          <div style={{ fontSize: '1rem', fontWeight: 800, color: '#334155', marginBottom: 6 }}>
            No hay plantillas disponibles
          </div>
          <div style={{ fontSize: '0.84rem', color: '#64748b', marginBottom: 16 }}>
  ...
  return (
    <div data-testid="templates-view" className="tv-root">
      <style>{`
        ...
        @keyframes tv-build-pulse{0%,100%{box-shadow:0 2px 8px rgba(0,75,147,.18),0 0 0 1px rgba(255,255,255,.12) inset}50%{box-shadow:0 4px 14px rgba(0,75,147,.26),0 0 12px rgba(59,130,246,.18)}}
        ...
        .tv-build-btn{position:relative;overflow:hidden;isolation:isolate;display:inline-flex;align-items:center;gap:6px;height:28px;padding:0 12px;border-radius:999px;border:1px solid #1e3a8a;background:linear-gradient(135deg,#0b2a5b 0%,#004B93 45%,#0e6ad1 100%);color:#fff;font-size:0.68rem;font-weight:800;cursor:pointer;white-space:nowrap;transition:transform 180ms var(--ease),box-shadow 180ms var(--ease),background 180ms var(--ease);animation:tv-build-pulse 3s ease-in-out infinite}
        .tv-build-btn::before{content:'';position:absolute;inset:0;background:linear-gradient(105deg,transparent 30%,rgba(255,255,255,.22) 46%,rgba(255,255,255,.34) 50%,transparent 62%);transform:translateX(-140%) skewX(-12deg);animation:tv-build-shine 3.4s ease-in-out infinite;pointer-events:none}
        .tv-build-btn:hover{transform:translateY(-1px);background:linear-gradient(135deg,#0d335f 0%,#0054a6 45%,#1280e0 100%);box-shadow:0 4px 14px rgba(0,75,147,.3)}
        .tv-build-btn:active{transform:translateY(0) scale(.97)}
        .tv-build-btn:focus-visible{outline:2px solid #93c5fd;outline-offset:2px}
        @media (prefers-reduced-motion:reduce){.tv-build-btn,.tv-build-btn::before{animation:none}.tv-build-btn:hover{transform:none}}
      `}</style>
      ...
      <div className="tv-header">
        ...
        <div className="tv-header-right">
          <button
            ...
            data-testid="tv-build-template-btn"
            title="Aprende a crear tu propia plantilla de Word"
            aria-label="Construir mi plantilla — abre guía interactiva"
            className="tv-build-btn"
          >
            <svg
              width="13"
              height="13"
          ...
          <Button
            variant="ghost"
            onClick={() => goTo('datos')}
            data-testid="tv-volver"
            style={{ fontSize: '0.68rem', height: 28, padding: '0 10px' }}
          >
            Volver
          </Button>
          <Button
            ...
            style={{ fontSize: '0.68rem', height: 28, padding: '0 12px' }}
          >
      ...
      <div className="tv-summary" data-testid="tv-summary">
        <span className="tv-summary-text">
          ...
          <strong style={{ color: pendingCount > 0 ? 'var(--warning)' : 'var(--neutral-500)' }}>
            {pendingCount}
          </strong>{' '}
          pendientes
        </span>
      ...
      <div className="tv-main" data-testid="tv-layout">
        ...
        <div className="tv-left" data-testid="tv-templates-panel">
          <div className="tv-left-hdr">
            ...
            <Badge variant="info" style={{ fontSize: '0.55rem' }} data-testid="tv-count">
          ...
          {categories.length > 0 && (
            <div className="tv-left-cats" data-testid="tv-categories">
              <button
                ...
                data-testid="tv-cat-todas"
                className={`tv-chip ${selectedCategory === 'todas' ? 'active' : 'inactive'}`}
              >
                Todas
              </button>
              {categories.map((cat) => (
                <button
                  ...
                  data-testid={`tv-cat-${cat}`}
                  className={`tv-chip ${selectedCategory === cat ? 'active' : 'inactive'}`}
                >
                  {cat}
                </button>
              ))}
          ...
          <div className="tv-left-body" ref={templateListRef} data-testid="tv-list">
            {filteredTemplates.length === 0 ? (
              ...
              paginatedTemplates.map((tpl) => {
                ...
                const count = Object.values(templateAssignments).filter(
                  (tid) => tid === tpl.id
                ).length;
                return (
                  <button
                    ...
                    className={`tv-tpl ${isActive ? 'active' : ''}`}
                    style={{
                      animation: 'tv-cardIn 280ms var(--ease) both',
                      borderColor: isActive ? '#004B93' : undefined,
                    }}
                  >
                    <span
                      className="tv-tpl-icon"
                      style={{
                        background: isActive ? '#dbeafe' : 'var(--neutral-100)',
                        color: isActive ? 'var(--essa-primary)' : 'var(--neutral-500)',
                      }}
                    >
                      <svg
                        width="10"
          ...
          {filteredTemplates.length > TEMPLATE_PAGE_SIZE && (
            <div className="tv-pagination" data-testid="tv-pagination">
              ...
              <span className="tv-pagination-text" data-testid="tv-page-indicator">
        ...
        <div className="tv-center" data-testid="tv-preview-panel">
          ...
          {!selectedTemplate && (
            <div data-testid="tv-preview-empty" className="tv-viewer-empty" style={{ height: 500 }}>
              ...
              <div style={{ fontSize: '0.75rem', color: 'var(--neutral-400)', marginTop: 4 }}>
                Elige una plantilla del panel izquierdo para previsualizar
              </div>
            </div>
          )}
          ...
          {selectedTemplate && (
            <div
              className="tv-vars"
              style={{ maxHeight: varsExpanded ? 120 : 28 }}
              data-testid="tv-variables"
            >
              <button
                type="button"
                className="tv-vars-toggle"
              ...
              >
                ...
                <svg
                  ...
                  style={{
                    ...
                    transform: varsExpanded ? 'rotate(180deg)' : 'rotate(0deg)',
                    color: 'var(--neutral-400)',
                  }}
                >
              ...
              <div className="tv-vars-body">
                {!selectedTemplate.variables || selectedTemplate.variables.length === 0 ? (
                  <div
                    style={{ fontSize: '0.68rem', color: 'var(--neutral-400)' }}
        ...
        <div className="tv-right" data-testid="tv-records-panel">
          <div className="tv-right-hdr">
            ...
            <Badge variant="info" style={{ fontSize: '0.55rem' }} data-testid="tv-record-count">
          ...
          <div className="tv-right-filters">
            {(['todos', 'asignado', 'sin_asignar'] as const).map((f) => (
              <button
                ...
                className={`tv-chip ${recordStatusFilter === f ? 'active' : 'inactive'}`}
                data-testid={`tv-filter-${f}`}
              >
            ...
            {filteredRecords.length === 0 ? (
              ...
              <>
                  ...
                  <input
                    ...
                    checked={
                      ...
                      filteredRecords.every((r) =>
                        recordCheckboxes.has((r as unknown as { rowId: string }).rowId)
                      )
                    }
                    onChange={handleToggleAllRecords}
                    style={{ accentColor: 'var(--essa-primary)' }}
                  />
                ...
                {filteredRecords.map((r) => {
                  ...
                  const assignedTpl = assignedTid
                    ? templates.find((t) => t.id === assignedTid)
                    : null;
                  const isChecked = recordCheckboxes.has(rowId);
                  return (
                    <div
                      key={rowId}
                      className={`tv-rr ${isChecked ? 'sel' : ''}`}
                      onClick={() => handleToggleRecord(rowId)}
                      data-testid={`tv-record-${rowId}`}
                    >
                      <input
                        type="checkbox"
                        checked={isChecked}
                        onChange={() => handleToggleRecord(rowId)}
                        onClick={(e) => e.stopPropagation()}
                        style={{ accentColor: 'var(--essa-primary)', flexShrink: 0 }}
                      />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div className="tv-rr-name" title={nombre}>
                          {nombre}
                        </div>
                        <div className="tv-rr-meta">
                          <span>Cta:{cuenta}</span>
                          <span>Rad:{radicado}</span>
                        </div>
                        {assignedTpl ? (
                          <div style={{ marginTop: 1 }}>
                            ...
                            >
                              ✓ {assignedTpl.title || assignedTpl.fileName}
                            </span>
                          </div>
                        ) : (
                          <div style={{ marginTop: 1 }}>
          ...
          <div className="tv-right-footer">
            <Button
              variant="primary"
              disabled={!selectedTemplate || recordCheckboxes.size === 0}
              onClick={handleAssignTemplate}
              data-testid="tv-assign-btn"
              style={{ width: '100%', fontSize: '0.68rem' }}
            >
              {selectedTemplate && recordCheckboxes.size > 0
                ? `Asignar a ${recordCheckboxes.size} registro${recordCheckboxes.size !== 1 ? 's' : ''}`
                : 'Selecciona plantilla y registros'}
            </Button>
          </div>
        </div>
      </div>
      ...
      {confirmOverwrite.open && (
        <div
          ...
          data-testid="tv-confirm-overlay"
          onClick={() => setConfirmOverwrite({ open: false, rowIds: [], templateId: '' })}
        >
          <div
            style={{
              background: '#fff',
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <Button
                ...
                data-testid="tv-confirm-overwrite"
              >
                Reemplazar
              </Button>
            </div>
