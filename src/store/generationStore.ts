...
export const useGenerationStore = create<GenerationStore>()(
  persist(
    (set, get) => ({
      ...
      addExcludedId: (id) =>
        set((s) => {
          if (s.excludedIds.includes(id)) return s;
          return { excludedIds: [...s.excludedIds, id] };
        }),
      removeExcludedId: (id) =>
        set((s) => ({
      ...
      clearGenerated: () =>
        set((s) => ({
          docResults: [],
          stage: 'revision' as GenerationStage,
          progress: 0,
          // Preservar excludedIds: los documentos eliminados individualmente permanecen eliminados
    ...
    {
      name: 'essa-generation',
      storage: createJSONStorage(() => localStorage),
      partialize: (state) => ({ excludedIds: state.excludedIds }),
    }
  )
);
